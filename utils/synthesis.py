...
def synthesis(m, s, CONFIG, use_cuda, ap):
    ...
    world_feature, alignments, stop_tokens = m.forward(
        chars_var.long())
    world_feature = world_feature[0].data.cpu().numpy()
    alignment = alignments[0].cpu().data.numpy()
    wav_w = ap.world2wav(world_feature)
    wav = ap.inv_spectrogram(wav_w.T)
    wav = wav[:ap.find_endpoint(wav)]
    return wav, alignment, stop_tokens