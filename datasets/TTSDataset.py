...
class MyDataset(Dataset):
    ...
    def load_data(self, idx):
        ...
        sample = {'text': text, 'wav': wav, 'item_idx': self.items[idx][1], 'world':world, 'linear': linear}
        return sample
    ...
    def collate_fn(self, batch):
        ...
        if isinstance(batch[0], collections.Mapping):
            ...
            if batch[0]['world'] is None:
                world = [self.ap.wav2world(w).astype('float32') for w in wav]
            else:
                world = [d['world'] for d in batch]
            world_lengths = [m.shape[1] + 1 for m in world]  # +1 for zero-frame

            # compute 'stop token' targets
            stop_targets = [
                np.array([0.] * (world_len - 1)) for world_len in world_lengths
            ]
            ...
            world = prepare_tensor(world, self.outputs_per_step)
            assert world.shape[2] == linear.shape[2]
            timesteps = world.shape[2]

            # B x T x D
            linear = linear.transpose(0, 2, 1)
            world = world.transpose(0, 2, 1)

            # convert things to pytorch
            text_lenghts = torch.LongTensor(text_lenghts)
            text = torch.LongTensor(text)
            linear = torch.FloatTensor(linear).contiguous()
            world = torch.FloatTensor(world).contiguous()
            world_lengths = torch.LongTensor(world_lengths)
            stop_targets = torch.FloatTensor(stop_targets)

            return text, text_lenghts, linear, world, world_lengths, stop_targets, item_idxs
