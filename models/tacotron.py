...
class Tacotron(nn.Module):
    ...
    def forward(self, characters, mel_specs=None, mask=None):
        ...
        world_outputs, alignments, stop_tokens = self.decoder(
            encoder_outputs, mel_specs, mask)
        # Reshape
        # batch x time x dim
        world_outputs = world_outputs.view(B, -1, self.mel_dim)
        #linear_outputs = self.postnet(mel_outputs)
        world_outputs = self.last_linear(world_outputs)
        return world_outputs, alignments, stop_tokens
